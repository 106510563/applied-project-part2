...
<body>
        ...
        <?php
        $conn = mysqli_connect('localhost', 'root', '', 'linkly_db');
        ...
        <form method="GET" action="" style="display: grid;width: 360px;margin: auto;">
                ...
                <select class="filter" name="sort_field">
                        <?php foreach ($allowed_sort as $f): ?>
                                <option value="<?php echo $f; ?>" <?php if ($sort_field === $f)
                                           echo "selected"; ?>>
                ...
                <button type="reset"
                        style="border: 2px inset black;border-radius: 3px;background: #d0d0d4;text-align: center;margin: 10px;padding: 3px;">Reset</button>
        </form>
        ...
        <form method="POST" action=""
                ...
                <input type="text" class="filter" id="del_jobref" name="del_jobref" placeholder="e.g. ECC01" required>
        ...
        if (mysqli_num_rows($result) > 0) {
                echo "<table id='eoitable' border='1' cellpadding='5' style='right:0px; position:absolute;'>";
